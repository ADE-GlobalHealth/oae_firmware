...
HAL_StatusTypeDef TLV320ADC3120_Initialize(TLV320ADC3120 *dev, I2C_HandleTypeDef *i2cHandle){
	...
	status = TLV320ADC3120_WriteRegister(dev, 0x01, &(uint8_t){0x01});
	errNum += (status != HAL_OK);
	HAL_Delay(20);

	status11 = TLV320ADC3120_ReadRegister(dev, 0x02, &reg_data_1);

	status = TLV320ADC3120_WriteRegister(dev, 0x02, &(uint8_t){0x05});
	errNum += (status != HAL_OK);
	...
	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_INPUT, &(uint8_t){0xF0});
	errNum += (status != HAL_OK);

	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_SAI, &(uint8_t){0xF0});
	errNum += (status != HAL_OK);

	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_PLL, &(uint8_t){0xE0});
	errNum += (status != HAL_OK);
	HAL_Delay(10);

	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_ASI_CONFIG, &(uint8_t){0x60});
	errNum += (status != HAL_OK);

	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_MCLK_CONFIG, &(uint8_t){0x80});
	errNum += (status != HAL_OK);

	status = TLV320ADC3120_WriteRegister(dev, TLV320ADC3120_SAMPLE_CONFIG, &(uint8_t){0x48});
	errNum += (status != HAL_OK);
